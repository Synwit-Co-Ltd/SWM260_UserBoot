...
*			uint32_t period			对于TIMR0~3: 定时/计数周期，32位
*									对于BTIMR0~3: 低24位period[23:0]为定时周期，高8位period[31:24]为预分频值
...
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，可取值包括TIMR0、TIMR1、TIMR2、TIMR3、BTIMR0、BTIMR1、BTIMR2、BTIMR3
*			uint32_t period			对于TIMR0~3: 定时/计数周期，32位
*									对于TIMR4~7: 低24位period[23:0]为定时周期，高8位period[31:24]为预分频值
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
* 功能说明:	获取定时/计数周期
* 输    入: TIMR_TypeDef * TIMRx	指定要被设置的定时器，可取值包括TIMR0、TIMR1、TIMR2、TIMR3、BTIMR0、BTIMR1、BTIMR2、BTIMR3
* 输    出: uint32_t				对于TIMR0~3: 定时/计数周期，32位
*									对于TIMR4~7: 低24位为定时周期，高8位为预分频值
* 注意事项: 无
******************************************************************************************************************************************/
