...
* 函数名称:	UART_Init()
* 功能说明:	UART串口初始化
* 输    入: UART_TypeDef * UARTx	指定要被设置的UART串口，有效值包括UART0、UART1、UART2、UART3、UART4
...
{	
	...
	if((UARTx == UART0) || (UARTx == UART1) || (UARTx == UART2) || (UARTx == UART3))
		...
		UARTx->BAUD |= (((SystemCoreClock/2)/16/initStruct->Baudrate - 1) << UART_BAUD_BAUD_Pos);
	...
	case ((uint32_t)UART4):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn)
		{
			NVIC_EnableIRQ(PWM2_UART4_IRQn);
		}
		else
		{
			NVIC_DisableIRQ(PWM2_UART4_IRQn);
		}
		break;
	}
}
...
* 函数名称:	UART_WriteByte()
* 功能说明:	发送一个字节数据
* 输    入: UART_TypeDef * UARTx	指定要被设置的UART串口，可取值包括UART0、UART1、UART2、UART3、UART4、UART4
*			uint8_t data			要发送的字节			
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
void UART_SetBaudrate(UART_TypeDef * UARTx, uint32_t baudrate)
{
	...
	if((UARTx == UART0) || (UARTx == UART1) || (UARTx == UART2) || (UARTx == UART3))
		...
		UARTx->BAUD |= (((SystemCoreClock/2)/16/baudrate - 1) << UART_BAUD_BAUD_Pos);
}
